...
const ProductCard = ({ product }) => {
  ...
  return (
    <div className="group bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col h-full">
      ...
      {/* Content */}
      <div className="py-5 px-2 md:px-4 flex-1 flex flex-col">
        <div className="mb-2">
          ...
          <p className="text-sm text-slate-400 font-medium mb-2">
            {product.device?.toUpperCase() || "Security System"}
          </p>
        </div>

        <p className="text-sm text-slate-600 line-clamp-4 mb-4 flex-1 leading-relaxed">
          {product.description ||
            "High quality security solution for your protection needs."}
        </p>
